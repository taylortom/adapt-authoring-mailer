...
class MailerModule extends AbstractModule {
  ...
  async transporterInit () {
    this.transporter = new EmailClient(this.connectionUrl)
  }

  async transporterSend (data) {
    const POLLER_WAIT_TIME = this.getConfig('pollerWaitTime')
    const message = {
    ...
    const poller = await this.transporter.beginSend(message)
  ...
  async send (data, options = {}) {
    ...
    try {
      ...
      await this.transporterSend(data)
