 ...
 */
export default class MailerModule extends AbstractModule {
  /** @override */
  async init() {
    /**
   ...
   */
  async send(message) {
    if (!this.isEnabled || !this.transporter || !this.isConnectionVerified) {
    ...
    try {
      this.log('info', `email send successfully, ${(await this.transporter.sendMail(message)).response}`);
    } catch(e) {
