...
class MailerModule extends AbstractModule {
  ...
  validateConfig() {
    if (this.useConnectionUrl && !MailerUtils.isValidSmtpConnectionUrl(this.connectionUrl)) {
      this.log('error', this.app.lang.t("error.smptUrlValidationFailed", {url: this.connectionUrl}));
      return false;
    } else if (
      !MailerUtils.isValidPort(this.port) ||
      !MailerUtils.isValidEmail(this.from) ||
      !this.host ||
      !this.user
    ) {
      this.log('error', this.app.lang.t("error.smptParamsValidationFailed", {url: this.connectionUrl}));
      return false;
    }
  ...
  async send(to, subject, text, html) {
    ...
    if (MailerUtils.isValidEmail(to)) {
      ...
      this.log('warn', 'Mailer#send()',
        this.app.lang.t("error.smptEmailValidationFailed", {email: to}));
    }
  }
}
